    ...
    return (
        <div className='card-container'>
            <Link className='card-value' to={`/single/${type}/${singleResult.id}`}>
                ...
                {type === 'movie' ? `${(singleResult.title.length > 20) ? singleResult.title.slice(0, 11) + "..." : singleResult.title}` : `${(singleResult.name.length > 20) ? singleResult.name.slice(0, 16) + "..." : singleResult.name}`}
            </Link>
